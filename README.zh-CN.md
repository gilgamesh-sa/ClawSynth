...
其中，`soft_verify` 验证模块会结合 query、workspace 状态和 agent 最终回复，生成验证计划并输出 `pass` / `review` / `fail` 结果。详细说明见 [src/soft_verify/README.zh-CN.md](./src/soft_verify/README.zh-CN.md)。

## 环境配置

本项目使用 Python `3.13.x`，推荐使用 `uv` 管理环境。

如果本机还没有 `uv`，可以先安装 `uv`。安装完成后，在项目根目录执行：
...
如果你希望启用 LiteLLM 网关抓取到底层模型最原始的请求和返回结果，还需要额外参考 [litellm_config/README.zh-CN.md](./litellm_config/README.zh-CN.md) 完成 LiteLLM 配置，并让 OpenClaw 改为通过 LiteLLM 代理调用模型。
...
解压后每个 skill 目录下都应包含 `SKILL.md`。如果这些目录已经存在，可以不用重复解压。

**你也可以继续添加更多可用 skills 来丰富采样数据的类型。**
...
- `src/gen_query/README.zh-CN.md`：query 合成阶段的详细说明。
- `src/README.zh-CN.md`：反向文件生成和正向轨迹生成两个 OpenClaw 阶段的详细说明。
- `src/soft_verify/README.zh-CN.md`：对话轨迹验证模块的详细说明，包括 LiteLLM 日志整理、验证输入抽取和两步验证流程。
- `litellm_config/README.zh-CN.md`：LiteLLM 网关抓取的配置说明，用于保留底层模型原始请求和返回结果。
...
如果你希望获取到底层模型最原始的请求和返回结果，而不只是最终 session 轨迹文件，那么必须启用 `litellm_config/`，让 OpenClaw 通过 LiteLLM 网关调用模型。否则默认流程只能拿到 OpenClaw 导出的 session 轨迹，拿不到最底层的原始请求数据。详细说明见 [litellm_config/README.zh-CN.md](./litellm_config/README.zh-CN.md)。

启用 LiteLLM 后，原始抓取结果会写入 `litellm_config/success_events_all.jsonl`。如果你想把它转换成更易读的对话格式，可以执行：
...
- `checkpoint.jsonl`
- `summary.json`
- 每个 workspace 或 domain 下的 OpenClaw session 轨迹文件
...
最终会得到带有 `verdict`、`score` 和 `llm_check_results` 的验证结果。完整说明见 [src/soft_verify/README.zh-CN.md](./src/soft_verify/README.zh-CN.md)。
...
- [src/gen_query/README.zh-CN.md](./src/gen_query/README.zh-CN.md)
- [src/README.zh-CN.md](./src/README.zh-CN.md)
- [litellm_config/README.zh-CN.md](./litellm_config/README.zh-CN.md)
- [src/soft_verify/README.zh-CN.md](./src/soft_verify/README.zh-CN.md)
